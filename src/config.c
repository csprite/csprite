...
Config_T* LoadConfig(void) {
	...
	if (access(configPath, F_OK) == 0) {
		...
		if (vsync == NULL) s->vsync = false;
		else if (strncmp(vsync, "true", 5) == 0) s->vsync = true;
		else s->vsync = false;

		const char* FramesUpdateRate_Str = ini_get(config, "csprite", "FramesUpdateRate");
		s->FramesUpdateRate = FramesUpdateRate_Str == NULL ? 60 : atoi(FramesUpdateRate_Str);
